# 靡不有初，鲜克有终
# 开发时间：2022/10/26 9:44
"""
演示非线性FR共轭梯度法解决一个四次函数的最优化问题的例子
优化函数：四次函数，n=2
...
def FRconjugate_gradient_method(vector_initial, epsilon):  # 共轭梯度法
    ...
    while float(gradient(vector_x_pre)[1]) > epsilon and k <= 100:
        ...
        beta = np.dot(gradient(vector_x_new)[0].T, gradient(vector_x_new)[0]).item()/np.dot(gradient(vector_x_pre)[0].T, gradient(vector_x_pre)[0]).item()  # FR
...
def plot_2D_figure(X1, X2, F, x_1_lst, x_2_lst):  # 绘制二维图像并保存
    ...
    plt.savefig('C://Users//张晨皓//Desktop//最优化基础理论与方法学习代码//图片//11.非线性FR共轭梯度法2D.png')
    plt.show()
...
def plot_3D_figure(X1, X2, F, x_1_lst, x_2_lst,f_value_lst):  # 绘制三维图像并保存
    ...
    plt.savefig('C://Users//张晨皓//Desktop//最优化基础理论与方法学习代码//图片//11.非线性FR共轭梯度法法3D.png')
    plt.show()
