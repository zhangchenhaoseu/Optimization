# 靡不有初，鲜克有终
# 开发时间：2022/10/25 9:25
"""
演示DFP拟牛顿法解决一个凸函数的最优化问题的例子
优化函数：三次函数，n=2
...
def DFP_quasi_newton_method(vector_initial, epsilon):  # DFP拟牛顿法
    ...
    while float(gradient(vector_x_pre)[1]) > epsilon and k <= 100:
        ...
        '''if n == 10:  # n 步重启
            ...
            H_pre = np.linalg.inv(B_pre)
            n = 0'''

        vector_x_new = np.array(vector_x_pre - np.dot(H_pre, gradient(vector_x_pre)[0]))
        ...
        H_new = H_pre - (H_pre*y*y.T*H_pre)/(np.dot(y.T, np.dot(H_pre, y))).item() + (s*s.T)/np.dot(y.T,s).item()  # 如果1步重启的话这个式子压根就不会起到效果
        H_pre = H_new
...
if __name__ == "__main__":
    ...
    vector_x_opt, value, times, x_1_lst, x_2_lst, f_value_lst = DFP_quasi_newton_method(vector, 0.001)
