# 靡不有初，鲜克有终
# 开发时间：2022/10/25 15:47

"""
演示SR-1拟牛顿法解决一个凸函数的最优化问题的例子
优化函数：三次函数，n=2
...
def SR1_quasi_newton_method(vector_initial, epsilon):  # SR-1拟牛顿法
    k = 0
    n = 0  # n步重启
    vector_x_pre = vector_initial
    B_initial = hesse(vector_x_pre)  # B_0的计算（或者修正）要体现一些二阶信息，计算成本也要低，当然信息越多越好
    B_pre = B_initial  # B_0
    ...
    while float(gradient(vector_x_pre)[1]) > epsilon and k <= 300:
        ...
        B_new = B_pre + np.dot((y-np.dot(B_pre, s)),(y-np.dot(B_pre, s)).T)/np.dot((y-np.dot(B_pre,s)).T,s).item()
        B_pre = B_new
...
def plot_2D_figure(X1, X2, F, x_1_lst, x_2_lst):  # 绘制二维图像并保存
    ...
    plt.savefig('C://Users//张晨皓//Desktop//最优化基础理论与方法学习代码//图片//9.SR1拟牛顿法2D求解四次函数.png')
    plt.show()
...
def plot_3D_figure(X1, X2, F, x_1_lst, x_2_lst,f_value_lst):  # 绘制三维图像并保存
    ...
    plt.savefig('C://Users//张晨皓//Desktop//最优化基础理论与方法学习代码//图片//9.SR1拟牛顿法3D求解四次函数.png')
    plt.show()


if __name__ == "__main__":
    vector = np.array([[-0.5], [1.5]])
    vector_x_opt, value, times, x_1_lst, x_2_lst, f_value_lst = SR1_quasi_newton_method(vector, 0.001)
