...
def step_length(x_1, x_2, search_dir, K, B):  # 线搜索确定步长 x2 = Kx1 + B
    x1 = sp.Symbol('x1')
    x2 = sp.Symbol('x2')
    if search_dir == 1:  # 搜索方向x_1轴
        ...
        return step
    if search_dir == 2:  # 搜索方向x_2轴
        x_2_function = sp.solve(gx2, x2)[0]
        step = x_2_function.evalf(subs={x1: x_1}) - x_2
        return step
    elif search_dir == 3:  # 搜索方向是x_2和x_1的合成方向
        x1 = sp.Symbol('x1')
        x_3_function = pow(x1, 2) + 2 * pow((K*x1+B), 2) + 2 * x1 * (K*x1+B)-5
        x1_solve = float(sp.solve(sp.diff(x_3_function, x1), x1)[0])
        x2_solve = (K*x1+B).evalf(subs={x1: x1_solve})
        step_x1 = float(x1_solve - x_1)
        step_x2 = float(x2_solve - x_2)
...
def coordinate_descent_method(start_x_1, start_x_2, epsilon):  # 交替坐标轴下降算法
    ...
    while gradient_modulus(x_1, x_2) > epsilon and k <= 100:
        ...
        search_dir = 1
        step_x_1 = step_length(x_1, x_2, search_dir, 0, 0)
        x_1 = x_1 + step_x_1
        x_1_lst.append(x_1)
        x_2_lst.append(x_2)
        ...
        search_dir = 2
        step_x_2 = step_length(x_1, x_2, search_dir, 0, 0)
        x_2 = x_2 + step_x_2
        x_1_lst.append(x_1)
        x_2_lst.append(x_2)
        f_value_lst.append(function_value(x_1, x_2))
        ...
        search_dir = 3
        K = (x2_new-x2_pre)/(x1_new-x1_pre)
        B = x2_pre - x1_pre * K
        step_x1, step_x2 = step_length(x_1, x_2, search_dir, K, B)
        x_1 = x_1 + step_x1
        x_2 = x_2 + step_x2
        x_1_lst.append(x_1)
        x_2_lst.append(x_2)
...
def plot_2D_figure(X1, X2, F, x_1_lst, x_2_lst):  # 绘制二维图像并保存
    ...
    plt.savefig('C://Users//张晨皓//Desktop//最优化基础理论与方法学习代码//图片//2.坐标轴交替下降法2D（改进）.png')
    plt.show()
...
def plot_3D_figure(X1, X2, F, x_1_lst, x_2_lst,f_value_lst):  # 绘制三维图像并保存
    ...
    plt.savefig('C://Users//张晨皓//Desktop//最优化基础理论与方法学习代码//图片//2.坐标轴交替下降法3D（改进）.png')
    plt.show()


if __name__ == "__main__":
    x1_opt, x2_opt, value, times, x_1_lst, x_2_lst, f_value_lst = coordinate_descent_method(-5, 7, 0.01)  # 初始点
